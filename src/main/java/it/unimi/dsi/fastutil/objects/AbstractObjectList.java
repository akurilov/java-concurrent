...
public abstract class AbstractObjectList<K> extends AbstractObjectCollection<K>
		implements
	ObjectList<K>,
	Stack<K> {
	...
	public ObjectListIterator<K> listIterator(final int index) {
		...
		return new AbstractObjectListIterator<K>() {
			...
			public boolean hasNext() {
				return pos < it.unimi.dsi.fastutil.objects.AbstractObjectList.this.size();
			}
			...
			public K next() {
				...
				return it.unimi.dsi.fastutil.objects.AbstractObjectList.this.get(last = pos++);
			}
			public K previous() {
				if (!hasPrevious())
					throw new NoSuchElementException();
			...
			public void add(K k) {
				it.unimi.dsi.fastutil.objects.AbstractObjectList.this.add(pos++, k);
				last = -1;
			}
			public void set(K k) {
				if (last == -1)
					throw new IllegalStateException();
				it.unimi.dsi.fastutil.objects.AbstractObjectList.this.set(last, k);
			}
			public void remove() {
				if (last == -1)
					throw new IllegalStateException();
				it.unimi.dsi.fastutil.objects.AbstractObjectList.this.remove(last);
				/*
	...
	public void getElements(final int from, final Object a[], int offset,
			int length) {
		ObjectListIterator<K> i = listIterator(from);
		if (offset < 0)
			throw new ArrayIndexOutOfBoundsException("Offset (" + offset
	...
	public int compareTo(final List<? extends K> l) {
		...
		if (l instanceof ObjectList) {
			...
			final ObjectListIterator<K> i1 = listIterator(), i2 = ((ObjectList<K>) l)
					.listIterator();
			int r;
			K e1, e2;
	...
	public static class ObjectSubList<K> extends it.unimi.dsi.fastutil.objects.AbstractObjectList<K>
			implements
		...
		public void getElements(final int from, final Object[] a,
